<?php
    ...
    try
    {
        ...
        $select = $db->prepare("SELECT remarks , followuptemplatename , DATE_FORMAT(noteaddedon,'%d-%m-%Y %H:%i:%s') AS noteaddedon ,  DATE_FORMAT(nextfollowupdate,'%d-%m-%Y') AS nextfollowupdate FROM studentfollowup sf INNER JOIN followuptemplate ft ON sf.followuptemplateid = ft.followuptemplateid WHERE studentid = ? ORDER BY noteaddedon DESC");
        if($select == false)
        {
