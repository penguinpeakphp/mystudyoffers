<?php
    ...
    try
    {
        ...
        if($_SESSION["admintype"] == "admin")
        {
            ...
            $select = $db->prepare("SELECT queryid qi, querytopic , querytypeid qti , (SELECT querytypename FROM querytype WHERE querytypeid = qti) AS querytypename , (SELECT DATE_FORMAT(messagetime,'%d-%m-%Y %h:%i %p') FROM queryconversation WHERE queryid = qi ORDER BY conversationid DESC LIMIT 1) AS messagetime , (SELECT readbyadmin FROM queryconversation WHERE queryid = qi ORDER BY conversationid DESC LIMIT 1) AS readbyadmin , (SELECT adminid FROM queryconversation WHERE queryid = qi ORDER BY conversationid DESC LIMIT 1) AS adminid , sq.studentid , name FROM studentquery sq INNER JOIN student s ON sq.studentid = s.studentid ORDER BY messagetime DESC");   
        }
