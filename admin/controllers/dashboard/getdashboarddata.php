<?php
    ...
    try
    {
        ...
        //Fetch followups
        $response["followups"] = [];
        $result = $db->query("SELECT followupid , remarks , name , followuptemplatebody FROM studentfollowup sf INNER JOIN student s ON sf.studentid = s.studentid WHERE nextfollowupdate = '{$today}'");
        if($result == false)
        {
